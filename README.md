...
0.7.2는 즐겨찾기·새로고침·설정·업데이트·위젯 작업에 즉각적인 진행 및 완료 피드백을 더하고, 실수로 해제한 즐겨찾기를 복원할 수 있게 한 사용성 개선 릴리스다.

- 단일 홈 상단의 `햄버거 | 버스·정류장·장소 검색 | 내 주변`
- 최대 20개 즐겨찾기 정류장과 선택적 고정 노선
- 즐겨찾기 추가·해제, 노선 고정, 순서 저장, 수동 갱신 결과를 앱 전역 메시지로 안내
- 즐겨찾기 해제 직후 `실행 취소`로 고정 노선과 위젯 연결까지 함께 복원
- 정류장 전체 노선의 도착정보를 한 번에 조회하고 마지막 성공 시각과 함께 캐시
- 검색 결과를 버스·정류장·장소로 묶어 한 화면에 표시
- 검색어 지우기와 키보드 검색 완료 동작, 실제 저장 상태를 반영하는 즐겨찾기 버튼
...
- 위젯 연결·수동 새로고침의 진행 상태를 즉시 표시하고 실패 시 다시 시도 가능
...
0.6.x에서 처음 0.7.x로 갱신하면 기존 출근/퇴근 슬롯과 해당 도착 캐시·위젯 연결은 정리된다. 공공데이터 API 키와 재사용 가능한 노선·경로·차량 캐시는 유지된다. 0.7.0~0.7.2 사이에서 갱신할 때는 즐겨찾기와 위젯 연결을 그대로 유지한다.
